...
export function decomposePathsIntoRegions(
  ...
  for (const input of sourceInputs) {
    ...
    for (const region of currentRegions) {
      const intersectPath = region.path.copy()
      const okIntersect = intersectPath.op(inputPath, ck.PathOp.Intersect)

      const diffRegionPath = region.path.copy()
      const okDiff = diffRegionPath.op(inputPath, ck.PathOp.Difference)

      if (inputRemainder) {
        const nextRemainder = inputRemainder.copy()
        const okRem = nextRemainder.op(region.path, ck.PathOp.Difference)
        inputRemainder.delete()
        if (okRem && isPathNonEmpty(nextRemainder)) {
      ...
      if (okIntersect && isPathNonEmpty(intersectPath)) {
      ...
      if (okDiff && isPathNonEmpty(diffRegionPath)) {
        nextRegions.push({
          path: diffRegionPath,
...
export function initializeShapeBuilder(ctx: EditorContext): boolean {
  ...
  for (const node of selectedNodes) {
    ...
    nodePath.transform(worldTransform)

    if (isPathNonEmpty(nodePath)) {
      sourceInputs.push({ node, path: nodePath })
    } else {
      nodePath.delete()
    }
  }
...
function buildMergedVectorProp(
  ...
  const mergedPath = unwrapPath(draggedRegions[0].path).copy()
  for (let i = 1; i < draggedRegions.length; i++) {
    mergedPath.op(unwrapPath(draggedRegions[i].path), ck.PathOp.Union)
  }

  let result: VectorPropItem | null = null
  if (isPathNonEmpty(mergedPath)) {
    ...
    mergedPath.transform(ck.Matrix.translated(-minX, -minY))
    const vectorNetwork = parseSVGPath(mergedPath.toSVGString())
...
function buildNonDraggedVectorProps(
  ...
  for (const region of nonDraggedRegions) {
    ...
    const regionPath = rawPath.copy()
    const bounds = regionPath.getBounds()
    const minX = bounds[0]
    const minY = bounds[1]
    const width = bounds[2] - bounds[0]
    const height = bounds[3] - bounds[1]

    regionPath.transform(ck.Matrix.translated(-minX, -minY))
    const vectorNetwork = parseSVGPath(regionPath.toSVGString())
    regionPath.delete()
