...
function applyNodeVisualProps(
  ...
  if (node.effects.length > 0 && !hasRawUnsupportedEffects(node)) {
    const nativeEffects = node.effects.filter((effect) => isFigmaNativeEffect(effect))
    nc.effects = nativeEffects.map((effect) => ({
      type: (effect.type === 'LAYER_BLUR' ? 'FOREGROUND_BLUR' : effect.type) as Exclude<
        Effect['type'],
        'BRIGHTNESS_CONTRAST' | 'SATURATION' | 'CURVES' | 'NOISE'
      >,
...
export function sceneNodeToKiwiWithContext(
  ...
  syncAdjustmentEffectStackPluginData(node)
