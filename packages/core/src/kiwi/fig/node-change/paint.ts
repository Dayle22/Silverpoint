...
function applyGradientPaintFields(
  target: Pick<Fill, 'gradientStops' | 'gradientTransform'>,
  p: Paint
): void {
  if (!p.type.startsWith('GRADIENT') || !p.stops) return
  target.gradientStops = p.stops.map((s) => ({
    color: convertColor(s.color),
    position: s.position
  }))
  if (p.transform) target.gradientTransform = convertGradientTransform(p.transform)
}
...
export function convertStrokes(
  ...
  return paints.map((p) => {
    ...
    applyGradientPaintFields(stroke, p)
    return stroke
  })
}
