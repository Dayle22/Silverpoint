...
export const createShape = defineTool({
  ...
  description:
    'Create a basic node on the canvas of type FRAME, RECTANGLE, ELLIPSE, TEXT, LINE, STAR, POLYGON, or SECTION. Returns {id, name, type, width, height, ...}. Use create_vector for custom SVG shapes.',
...
export const createSlice = defineTool({
  ...
  description:
    'Create a slice node on the canvas to define a custom export region. Returns {id, name, type, ...}. Slices do not have visual styling.',
  execution: { kind: 'sync', mutation: 'document' },
  input: v.object({
    ...positionInputs,
