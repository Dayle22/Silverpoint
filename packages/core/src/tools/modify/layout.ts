...
export const setLayout = defineTool({
  ...
  description:
    'Configure auto-layout (flexbox) behaviour on a frame node. Returns {id, spacing} of the updated frame. You must pass a direction if the frame does not currently have auto-layout enabled.',
...
export const setConstraints = defineTool({
  ...
  description:
    'Define how a node resizes in relation to its parent frame. Returns {id, constraints}. This only works if the parent is a standard frame, not an auto-layout frame.',
  execution: { kind: 'sync', mutation: 'properties' },
  input: v.object({
    id: nodeIdInput,
