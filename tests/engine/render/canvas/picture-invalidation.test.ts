...
function pictureCache<T>(entries: Array<[string, T]>, dispose: (value: T) => void) {
  const cache = new BoundedLruCache<T>({ name: 'test', maxBytes: 1000, maxEntries: 100, dispose })
...
test('full picture invalidation resets tiled font-dependent resources', () => {
  ...
  const renderer = {
    ...
    nodePictureCache: pictureCache([['node', nodePicture]], (picture) => picture.delete()),
    ...
    subtreePictureCache: pictureCache([['subtree', { picture: subtreePicture }]], (entry) =>
